...
window.onload = function(){
    ...
    if(sysTag){//模拟器调试时使用(sysTag||isPhone) 打包时使用(sysTag)
        window.corNative = {
            ...
            //打开一个模态界面
            openModelWeb:function(params,id){
                 var iframe = document.createElement('iframe');
                 iframe.setAttribute('src',params.url);
                 console.log(params.frame.width+"--"+params.frame.height)
                 iframe.setAttribute('width',params.frame.width);
                 iframe.setAttribute('height',params.frame.height);
                 //iframe.setAttribute('scrolling',"no");
                 iframe.setAttribute('marginwidth',"0");
                 iframe.setAttribute('marginheight',"0");
                 iframe.setAttribute('frameborder',"0");
                 iframe.setAttribute('border',"0");
                 corJS.getDom(id).appendChild(iframe);
            },
            //返回上一个界面
            prevPage:function(){
                //window.history.go(-1);
...
var corjs = function(){
    ...
    //打开新窗口
    this.openWin = function(name,anim){
        if(anim==0){
            var type = "none";
        }else{
            var type = "push";
    ...
    //打开模态界面
    // 入参
    this.openModel = function(id,x,y,url){
        var params = {
            url:url+".html",
            frame:{
                x: x,            
                y: 44,            
                width: this.offset(id).w,      
                height: this.offset(id).h     
            }
        };
        params = this.checkParams(params);
        if(IsPC){
            corNative.openModelWeb(params,id);
        }else{
            corNative.openModelWeb(params);
        }
