...
;(function (window, undefined) {
  ...
  window.corOnload = function () {
    ...
    window.corNative = Object.assign(window.corNative || {}, {
      openFloatWindow: function (params) {
        var onTitleSet = params.onTitleSet;
        if (typeof onWebTitle === 'function') {
          // delete params.onWebTitle;  // 可能会删除引用对象的属性（副作用）
          corWindow.onWebTitle = onTitleSet;
          params.webTitle = 1;
        }
      ...
      windowHistoryBack: function (frameName) {
        corWindow.pageBack({
          name: frameName
        });
      },
      windowHistoryForward: function (frameName) {
        corWindow.pageForward({
          name: frameName
        });
      },
      //将指定窗口插入到另一窗口之上
      ...
      launchAndroidApp: function (mode, setting) {
        const PKG_LAUNCHER = 0;
        const ACTION_LAUNCHER = 1;
        // var pkgName, clsName, action, activity;      
        var main, add, opt, ex;
        var restArgs = Array.prototype.slice.call(arguments, arguments.callee.length);
        if (restArgs && restArgs.length > 0) {
          opt = restArgs.map(function (obj, i) {
            return JSON.stringify(obj);
          }).join(',');
        }
        switch (mode) {
          case PKG_LAUNCHER:
            main = setting.package;
            add = setting.className || '';
            ex.data = settting.data;
            ex.isNewTask = setting.isNewTask === false ? 0 : 1;
            ex = JSON.stringify(ex);
            break;
          case ACTION_LAUNCHER:
            main = setting.action;
            
            add.category = setting.categories;
            add.data = Object.create(null);
            add.data.mimeType = setting.mimeType;
            add.data.scheme = setting.scheme;
        ...
        return corWidget.startApp(mode, main, add, opt, ex);
      },
      ...
      launchWidget: function (id, options, cb) {
        ...
        var setting = {
          ...
          funcName: typeof onClosed === 'function' ?
            onClosed.name : undefined
        };
    
        var hasPath = false;
        var reg = /^\d+$/;    
        reg.test(id.toString()) && (hasPath = true)        

        if (typeof cb === 'function') {
          options.onLoaded = cb;
        }
        hasPath ? corWidget.startWidget(Object.assign(setting, {appId: id}), options.onLoaded)
          : corWidget.startSubWidget(Object.assign(setting, {path: id}), options.onLoaded)
      },
      /**
       * 退出一个子widget
       */
      closeWidget: function (id, options) {
        var idType = typeof id;
        if (idType === 'string' || idType === 'number') { // 第一参数必传
          var opt = Object.create(null);
          
          (!id || /^\d+$/.test(id.toString())) // 若定义第一参数，覆盖 options 传参
            ? (opt.appId = id ? id : undefined)
            : (opt.path = id)
    
          typeof options === 'object'
            ? Object.assign(opt, {
              resultInfo: options.message,
              animId: options.animateID
            }) // options 扩展 
        ...
        // 默认参数
        opt.animId = opt.animId || 5;
        // 调用函数
        opt.path ? corWidget.closeSubWidget(opt)
      ...
      requestAjax: function (params, callBack) {
        ...
        if (data && typeof data === 'object') { // Not undefined or null
          var queryStr = '';
          for (var key in data) {
            var value = data[key];
            queryStr += ('&'+ key +'=' + value);
        ...
        } else if (typeof data !== 'string' || method !== 'POST') { // only set POST request body while "String data"
          data = '';
        }
  ...
  function initCorNative() {
    ...
    window.corNative = {
      ...
      windowHistoryGo: function (step, frameName) {
        var global = frameName
          ? document.getElementById(frameName).contentWindow
          : window;
        
        global.history.go(step);
      },
      windowHistoryBack: function (frameName) {
        corNative.windowHistoryGo(-1, frameName);
      },
      windowHistoryForward: function (frameName) {
        corNative.windowHistoryGo(1, frameName);        
      },
      ...
      actionSheetDlg:function(params,callBack){
        ...
        for(var i=0;i<params.buttons.split(",").length;i++){
          html2+='<div class="co-tx-c" onclick = "" style="height:2.2rem;line-height:2.2rem;background:#fff;color:#000;border-bottom:1px #ccc solid">'+params.buttons.split(",")[i]+'</div>'
        }
