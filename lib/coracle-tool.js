...
export default {
  ...
  checkCoUI() {  
    coracleCmd.coracleCmd(homedir,['install','-g','cnpm','--registry=https://registry.npm.taobao.org'],'npm',false,null,function(){
        coracleCmd.coracleCmd(homedir,['i','-g','slush','slush-coui'],'cnpm',false,null,null);
    });
  },
  ...
  this.corWebView.addIpcMessageListener('create-CoUIModule', (data) => {
    ...
    coracleCmd.coracleCmd(null,['i','-g','slush',version],'cnpm',true,null,function(){
    ...
    dialog.showOpenDialog({
      ...
      text.on('navigate-to', (e, name) => {
            ...
            if(fs.existsSync(newPath)){
              ...
              that.consolePanel.notice(name+'工程重名了');
              return;
            }
           ...
           coracleCmd.coracleCmd(dir,['coui'],'slush',true,[name],function (){
            ...
            coracleCmd.coracleCmd(null,['config','set','production','false'],'npm',true,null,function(){
              
              atom.project.addPath(newPath);
              coracleCmd.coracleCmd(newPath,['i'],'npm',true,null,function(){
  ...
  this.corWebView.addIpcMessageListener('create-CoUIAddPage', (params) => {
     ...
     text.on('navigate-to', (e, name) => {
      ...
      if (global.coUIPageEvent) {
        ...
        const downloadUrl = package.host+'/cormobi/project/downPackage?filePath=/xpackage-cloud/upload_files/coui-page/'+params.pageType+'.vue';
    ...
    //     atom.notifications.addWarning('请输入名称');
    //     return;
    //  }
  ...
  opener (url) {
      ...
      if (urlInfo.protocol == "coracle-addpage:") {
        this.corWebView.setUri(package.host+"/cormobi/ide/add-page.html");
        return this.corWebView;
      }
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    runDist({event}) {
      ...
      coracleCmd.coracleCmd(projectPath,arg,cmdFirst,true,null,function(){
    ...
    buildCoUIModule({event}){
      ...
      text.on('navigate-to', (e, project) => {
        ...
        if(fs.existsSync(newPath)){
          atom.workspace.getActivePane().activate();
          atom.workspace.getLeftDock().show(); 
          that.consolePanel.notice(project+'功能重名了');
    ...
    publishLightApp({event}) {
      ...
      if(!appPath.startsWith(srcPath)||appPath == srcPath) {
       ...
       const deepPathArray = deepPath.split('/')
       if (deepPathArray.length>1) {
        const appName = deepPathArray[1];
        coracleCmd.coracleCmd(projectPath,['run','publish','-m',appName],'npm',true,null,function (){
          that.consolePanel.notice(appName+'发布成功');
          atom.notifications.addSuccess(appName+'发布成功');
        });
       } else {
        atom.notifications.addWarning('请选择要上传的轻应用文件夹');
       }
