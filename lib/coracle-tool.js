...
export default {
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    runDist({event}) {
      ...
      if(appPath.startsWith(srcPath)||appPath == srcPath) {
        ...
        if (deepPathArray.length>1) {
         ...
         coracleCmd.coracleCmd(projectPath,['run','dist','-m',appName],'npm',true,null,function (){
      ...
      coracleCmd.coracleCmd(projectPathB,arg,cmdFirst,true,null,function(){
    ...
    runBuild({event}) {
      ...
      if(appPath.startsWith(srcPath)||appPath == srcPath) {
        ...
        if (deepPathArray.length>1) {
         ...
         coracleCmd.coracleCmd(projectPath,['run','build','-m',appName],'npm',true,null,function (){
           that.consolePanel.notice(appName+'编译成功');
           atom.notifications.addSuccess(appName+'编译成功');
         });
         return;
    ...
    buildCoUIPage({event}){
      ...
      const pageInfo = Path.parse(dc);
      if (pageInfo.name !== 'page') {
        atom.notifications.addWarning('该目录不是标准的COUI的功能模块的page目录，无法添加页面!');
        return;
      }
