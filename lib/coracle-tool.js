...
export default {
  ...
  checkCoUI() {  
    ...
    if (!isDownLoadCoUI) {
      coracleCmd.coracleCmd(null,['-v'],'node',false,null,function(){
        coracleCmd.coracleCmd(homedir,['install','-g','cnpm','--registry=https://registry.npm.taobao.org'],'npm',true,null,function(error){
          if (error == 1) {
            ...
            atom.notifications.addWarning('检测到系统未安装node',{'dismissable':true,'detail':'请在安装node之后重启开发工具'});
          }
        });
      })
    }
