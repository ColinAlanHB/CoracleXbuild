...
export default {
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    realTimeLook(){
      ...
      console.log(selectedFileTreeView);
        if(selectedFileTreeView) {
            var path =  selectedFileTreeView.getAttribute('data-path');
            const fileURL = 'file://'+path;
            atom.notifications.addInfo(fileURL);        
            ipcRenderer.send ('realtime-look',path);
        } else {
            alert('else');
            ipcRenderer.send ('realtime-look','file:///users/hb/Desktop/test001/coracle02/src/index.html');
        }
    },
