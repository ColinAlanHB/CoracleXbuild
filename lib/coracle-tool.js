...
export default {
  ...
  this.corWebView.addIpcMessageListener('create-CoUIModule', (data) => {
    ...
    coracleCmd.coracleCmd(null,['i','-g','gulp','slush',version],'npm',true,null,null);
  });
      ...
      if (path != null) {
        ...
        coracleCmd.coracleCmd(dir,['coui'],'slush',true,['123'],function (){
          coracleCmd.coracleCmd(dir,['config','set','production','false'],'npm',true,null,function(){
            coracleCmd.coracleCmd(dir,['install'],'npm',true,null,null);
          });
          atom.workspace.getLeftDock().show();
          atom.project.addPath(dir);
        });
  ...
  /* 将指令解析为对应的参数与方法,指令与方法对应的规则为: 命令空间:方法名,参数1=值1,参数2=值2,
    ...
    runDist({event}) {
      ...
      coracleCmd.coracleCmd(projectPath,['run','dist'],'npm',true,null,null);
    },

    runDev({event}) {
      let projectPath = coracleFile.getCMDProjectPath({event:event});
      coracleCmd.coracleCmd(projectPath,['run','dev'],'npm',true,null,null);
    },

    runBuild({event}) {
      let projectPath = coracleFile.getCMDProjectPath({event:event});
      coracleCmd.coracleCmd(projectPath,['run','build'],'npm',true,null,null);
    },
