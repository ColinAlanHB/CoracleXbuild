	...
	if (pathArr.pop() === 'page') {
		fs.readFile(routePath,function(err, data) {
			...
			fs.writeFile(routePath, str, function(err){
					if (err) {
						return console.error('路由配置文件写入异常。');
					} else {
						return console.log('路由配置更新成功');
					}
				});
			}
		});
		} else {
		return console.error('该目录不是标准的COUI的功能模块的page目录，无法修改路由配置。');
		}
    },
